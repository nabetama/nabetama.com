...
## Route53 から Cloudflare に移行

### はじめに

自分は Route53 でドメインを管理していたが、Cloudflare でドメインを管理したいと思ったので移行した。
...
Route53ので、移管ロックをオフにする。自分も移管ロックがオンになっていたのでオフにした。
反映されるまで少し時間がかかる。

移管ができるようになると「Cloudflareに移管」のリンクがアクティブになる。

![移管が可能になった](https://i.imgur.com/8JSKe2pl.jpg)

上のリンクを遷移した先で、さきほどRoute53で発行した承認コードを入力する。
もしまだ支払い情報を入力してない場合、支払い情報を入力しないと次の画面に進めないので注意。
WHOIS 情報登録を行うとドメインの移管申請が完了する。
...
記載したSSL/TLS 設定で少しハマり、Aレコードでアクセスしたらリダイレクトループに陥ったが、無事に解決してドメインを Cloudflare で管理できるようになった。
同僚に聞いたところCloudflareはほぼ原価でドメインを管理しているとのことで長期的に見ればRoute53よりも料金面で優位性がある。
