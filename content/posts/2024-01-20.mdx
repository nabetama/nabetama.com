...
App Router 版の Next.js を使っているので`api/feed.xml/route.ts` を作成して実装した。
rssでは記事全文を配信することにしたので`generateFeed()` という関数を作り、
そこでrssに必要なデータを作成し、`route.ts`ではそれを受け取り、適切なヘッダーを付与して返却するだけ。
